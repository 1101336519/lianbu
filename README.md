...
- `SKILL.md`：Codex skill 版本，适合安装到 Codex skills 目录后用 `$lianbu` 调用。
- `UNIVERSAL_PROMPT.md`：通用提示词版本，适合复制给其他大语言模型，让它们调用自己的生图或修图模型执行同样流程。
...
下面示例展示 `lianbu` 的完整链路：原始全身图 -> 脸部裁切 -> 三视图生成 -> 彩铅重绘。

### 1. 输入原始模特图

![输入原始模特图](assets/examples/01-input-full-body.png)

### 2. 裁切脸部作为身份参考

![脸部裁切图](assets/examples/02-face-crop.png)

这一步会保留头发、发际线、耳朵、脸型、五官、下巴和少量颈部区域，作为后续三视图生成的身份锚点。

### 3. 生成同一人物三视图

![三视图生成图](assets/examples/03-three-view.png)

三视图包含正面、四分之三侧面和侧面。重点是保持同一人物的脸型、发型、眉眼比例、鼻嘴结构、耳环和整体气质一致。

### 4. 彩铅风格重绘

![彩铅重绘成品图](assets/examples/04-colored-pencil-final.png)

最终图保留三视图构图，并把照片质感转换为高质量彩铅手绘风格，白色背景，五官结构清晰可辨。
...
- 公开仓库中只提交明确用于示例展示的素材，不要提交用户隐私素材或临时生成草稿。
